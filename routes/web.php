...
Route::get('topup/tolak/{transaksi_id}', function ($transaksi_id) {
    ...
    return redirect()->back()->with("status", "Top Up di Tolak");
})->name('topup.tolak');
...
Route::get('bayar', function () {
    $datas = Transaksi::where("user_id", Auth::user()->id)->where("type", 2)->where("status",2);
    $total_data = 0;
...
Route::prefix('data_transaksi')->group(function (){
    ...
    Route::get('/data_detail_jajan', function ($id) {
        ...
        $transaksis = Transaksi::where("type", 2)
                                ->groupBy('invoice_id')
                                ->get();
        return view("data_detail_jajan",[
            "users" => $users,
            "transaksis" => $transaksis,
            "details" => $details,
        ]);
    })->name("data_detail_jajan");
...
Route::prefix('data_topup')->group(function (){
    ...
    Route::get('/detail_topup', function ($id) {
        $users =  Transaksi::where("user_id", Auth::user()->id)->get();
        $details = Transaksi::where("type", 1)
                            ->get();
